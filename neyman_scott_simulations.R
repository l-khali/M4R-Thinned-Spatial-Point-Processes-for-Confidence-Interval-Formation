source("neymann_scott_simulation_function.R")
source("method_1_version_2.R")
source("method_4_bootstrapping_spatial_processes.R")
source("method_3_version_5.R")

ns_tiling_4 <- ns_simulation_tiling(100, 250, 4, 0.05)
plot(ns_tiling_4[-1,], type="l", lty=3, ylim=c(0,1), main="Clustered: Tiling")
ns_tiling_16 <- ns_simulation_tiling(100, 250, 16, 0.05)
lines(ns_tiling_16[-1,], type="l", lty=2, ylim=c(0.5,1))
ns_tiling_64 <- ns_simulation_tiling(100, 250, 64, 0.05)
lines(ns_tiling_64[-1,], type="l", lty=1, ylim=c(0.5,1))

ns_approx_4 <- ns_simulation_approximation(100, 250, 4, 0.05)
plot(ns_approx_4[-1,], type="l", lty=3, ylim=c(0,1), main="Clustered: Splitting")
ns_approx_16 <- ns_simulation_approximation(100, 250, 16, 0.05)
lines(ns_approx_16[-1,], type="l", lty=2, ylim=c(0,1))
ns_approx_64 <- ns_simulation_approximation(100, 250, 64, 0.05)
lines(ns_approx_64[-1,], type="l", lty=1, ylim=c(0,1))

ns_subsets_4 <- ns_simulation_subsets5(100, 250, 4, 0.05)
plot(ns_subsets_4[-1,], type="l", lty=3, ylim=c(0,1), main="Clustered: Subsets")
ns_subsets_16 <- ns_simulation_subsets5(100, 250, 16, 0.05)
lines(ns_subsets_16[-1,], type="l", lty=2, ylim=c(0.5,1))
ns_subsets_64 <- ns_simulation_subsets5(100, 250, 64, 0.05)
lines(ns_subsets_64[-1,], type="l", lty=1, ylim=c(0.5,1))

ns_marked_4 <- ns_simulation_marked(100, 250, 4, 0.05)
plot(ns_marked_4[-1,], type="l", lty=3, ylim=c(0.5,1), main="Clustered: Marked point")
ns_marked_16 <- ns_simulation_marked(100, 250, 16, 0.05)
lines(ns_marked_16[-1,], type="l", lty=2, ylim=c(0.5,1))
ns_marked_64 <- ns_simulation_marked(100, 250, 64, 0.05)
lines(ns_marked_64[-1,], type="l", lty=1, ylim=c(0.5,1))
