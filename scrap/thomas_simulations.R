...
thomas_thinning_simulation <- function(nsim, thinning_param, alpha, kappa, scale, mu) {
  ...
  for (i in 1:length(Ws)) {
    ...
    for (sim in 1:nsim) {
      ...
      confidences <- thinning(p, thinning_param, alpha, W=Ws[i],r=c(0.0,0.05))
...
# the coverage resulting from these simulations is not good
# this is because we require an iid underlying distribution