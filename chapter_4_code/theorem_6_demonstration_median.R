...
for (thinning_param in c(0.2,0.5,0.8)) {
  ...
  for (n in ns) {
    ...
    true_median <- 50 + 1/3 - 1/(50*50)
    hist_2 <- c()
    for (sim in 1:R) {
      new_data <- rpois(n, 50)
      lambda_hat <- mean(new_data)
      median_hat <- lambda_hat + 1/3 - 1/(50*lambda_hat)
      hist_2 <- c(hist_2, sqrt(length(new_data))*(median_hat - true_median))
    }
    
    # histograms to observe convergence in distribution
    standard <- hist(hist_2, plot = FALSE)
    thin_hist <- hist(thinned_estimates, plot = FALSE)
    if (n<1000) {
      plot(thin_hist, col =rgb(116/255,10/255,255/255,alpha=0.7), freq=FALSE,main="",xlab="",ylab="",cex.lab=1.5,ylim=c(0,0.1),xlim=c(-20,20))
...
legend(-105,0.44,legend = c("Thinned Estimates", "Observed Estimates"), col = c(rgb(116/255,10/255,255/255,alpha=0.7),rgb(43/255,206/255,72/255,alpha=0.7)), lwd = 5, horiz = TRUE, cex = 1.3, seg.len=1, bty = 'n',xpd=NA)
par(op)
