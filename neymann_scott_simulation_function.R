...
neyman_scott_actual_k <- function(r=seq(0.0, 0.14, 0.01), lambda=25, R=0.1) {
  ...
  for (radius in 1:length(r)) {
    ...
    if (rad > 2*R) {
      ...
      z <- rad/(2*R)
      #K_actual <- c(K_actual, pi*rad + (1/lambda)*(2+(1/pi)*((8*z^2-4)*acos(z)-2*asin(z)+4*z*sqrt((1-z^2)^3)-6*z*sqrt(1-z^2))))
      K_actual <- c(K_actual, pi*(rad^2) + (1/lambda)*(2 + (1/pi)*( ((8*(z^2) - 4)*acos(z)) - 2*asin(z) + 4*z*sqrt((1 - z^2)^3) - 6*z*sqrt(1 - (z^2)))))
    }
  }
  return(K_actual)
}
...
ns_simulation_tiling <- function(nsim, lambda, nregions, alpha) {
  r <- seq(0.0, 0.14, 0.01)
  K_actual <- neyman_scott_actual_k(r=r, lambda=lambda)
  cover <- rep(c(0),each=15)
  coverage <- cbind(r, cover)
  for (i in 1:nsim) {
    print(paste0("Current simulation:",i))
    # generating matérn cluster field
    data <- rMatClust(25,0.1,10, wim=owin(c(0,1),c(0,1)))
