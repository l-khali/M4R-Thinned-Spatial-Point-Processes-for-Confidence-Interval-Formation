...
subsets_method5 <- function(data, N, alpha, R=99) {
  ...
  for (sim in 1:R) {
    ...
    for (n in 1:N) {
      ...
      #print(paste0("with coeff", as.data.frame(k)["iso"]))
      #print(paste0("without coeff", as.data.frame(k)["iso"] * ni * (ni - 1) / a))
    }
    #return(k_vals_current)
    k_vals <- cbind(k_vals, rowSums(k_vals_current) / (ni_sum * (ni_sum - 1)))
    #print(paste0("vals",k_vals))
  }
...
#subsets64v5 <- poisson_simulation_subsets5(30,250,64,0.05)
